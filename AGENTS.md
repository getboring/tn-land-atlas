...
- **Tests:** Playwright (38 tests x 3 viewports, one mobile-only skip on the other two -> ~112 effective runs) + Vitest (326 unit tests across `src/lib/insights.ts`, `permalink.ts`, `lazyRetry.ts`, and the full `src/lib/build-fit/` suite: schemas, storage, geometry, map-layers, project-file, report)
...
- `npm test` (vitest), 326 unit tests across `src/lib/{insights,permalink,lazyRetry}.ts` and the full `src/lib/build-fit/` suite
